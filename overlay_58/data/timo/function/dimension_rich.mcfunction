...
execute if entity @p[nbt=!{Dimension:"timo:rich"}] run tag @p add runnerONE
execute if entity @p[tag=runnerONE,nbt=!{Dimension:"timo:rich"}] run execute in timo:rich run tp @p ~ ~ ~
...
execute if entity @p[tag=runner,nbt={Dimension:"timo:rich"}] run tag @p remove runner